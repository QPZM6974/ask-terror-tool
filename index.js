...
const fs = fsDef.promises;

const proxyFile = fsDef.readFileSync("./proxy.txt", "utf8");
const proxyList = proxyFile.split(/\r?\n/);

const proxyMode = "true"; //true or false
...
async function FUNC_TEST() {
    FUNC_QUESTION_ONE(1, "198617115", "TEST", 0);
};
...
async function FUNC_FOLLOW_ONE(_myProcesNumber, _requestId) {
    ...
    try {
        ...
        let signUpRes = await Axios({
            "method": "POST",
            "url": `${baseUrl}/sing_ups.php`,
            "data": form,
            "proxy": {
                "protocol": "http",
                "host": proxyList[_myProcesNumber].split(":")[0],
                "port": proxyList[_myProcesNumber].split(":")[1] * 1,
                //   auth: {
                //     username: 'fixie',
        ...
        let followRequest = await Axios.post(`${baseUrl}/query.php?query=22`, `num=${_requestId}`, {
            "headers": {
                "Cookie": sessionId
            }
        });
...
async function FUNC_QUESTION_ONE(_myProcesNumber, _requestId, _requestContent, _data_anonymity) {
    ...
    try {
        ...
        let questionRequest = await Axios({
            "method": "POST",
            "url": `${baseUrl}/query.php?query=100`,
            "headers": {
                "Cookie": sessionId
            },
            "data": `id=${_requestId}&content=${encodeURI(_requestContent)}&makarong_bat=-1&show_user=${_data_anonymity}`,
            "proxy": {
                "host": proxyList[_myProcesNumber].split(":")[0],
                "port": proxyList[_myProcesNumber].split(":")[1],
                "port": 80,
                //   auth: {
                //     username: 'fixie',
                //     password: FIXIE_TOKEN
